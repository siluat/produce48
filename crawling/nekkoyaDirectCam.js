...
process.on('unhandledRejection', error => {
  const message = '[ERROR][내꺼야] ' + error.message;
...
const crawling = async items => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.nekkoyaDirectCamUrl || !item.nekkoyaDirectCamUrl.length) {
      continue;
    }

    await page.goto(item.nekkoyaDirectCamUrl,  {waitUntil: 'networkidle2'});
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set nekkoyaView = :nv, nekkoyaLike = :nl, nekkoyaComment = :nc',
...
const reportComplete = async () => {
  var message = '프로듀스48 내꺼야 직캠 영상 관련 정보를 업데이트 했습니다.';
