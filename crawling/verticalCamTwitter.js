...
const crawling = async items => {
  console.info('세로캠 트위터 반응 크롤링 시작!');
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.verticalCamTwitterUrl || !item.verticalCamTwitterUrl.length) {
      continue;
    }

    await page.goto(item.verticalCamTwitterUrl,  {waitUntil: 'networkidle2'});
  ...
  console.info('세로캠 트위터 반응 크롤링 완료!');
};
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set verticalCamTwitterLike = :l',
...
const reportComplete = async () => {
  var message = '프로듀스48 30 연습생 세로캠 영상 트위터 관련 정보를 업데이트 했습니다.';
