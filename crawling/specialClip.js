...
process.on('unhandledRejection', error => {
  const message = '[ERROR][스페셜 영상 네캐] ' + error.message;
...
const crawling = async items => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.specialClipUrl || !item.specialClipUrl.length) {
      continue;
    }

    await page.goto(item.specialClipUrl,  {waitUntil: 'networkidle2'});
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set specialClipView = :v, specialClipLike = :l, specialClipComment = :c',
