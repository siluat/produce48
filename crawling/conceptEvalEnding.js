...
process.on('unhandledRejection', error => {
  const message = '[ERROR][콘셉트 평가 엔딩 요정] ' + error.message;
...
const crawling = async items => {
  console.info('콘셉트 평가 엔딩 요정 영상 반응 크롤링 시작!');
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.endingUrl || !item.endingUrl.length) {
      continue;
    }

    await page.goto(item.endingUrl,  {waitUntil: 'networkidle2'});
  ...
  console.info('콘셉트 평가 엔딩 요정 영상 반응 크롤링 완료!');
};
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set endingView = :v, endingLike = :l, endingComment = :c',
...
const reportComplete = async () => {
  var message = '콘셉트 평가 엔딩 요정 영상 반응 정보를 업데이트 했습니다.';
