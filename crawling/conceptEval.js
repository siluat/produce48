...
process.on('unhandledRejection', error => {
  const message = '[ERROR][콘셉트 평가] ' + error.message;
...
const crawling = async items => {
  console.info('콘셉트 평가 영상 반응 크롤링 시작!');
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.conceptEvalUrl || !item.conceptEvalUrl.length) {
      continue;
    }

    await page.goto(item.conceptEvalUrl,  {waitUntil: 'networkidle2'});
  ...
  console.info('콘셉트 평가 영상 반응 크롤링 완료!');
};
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set conceptEvalView = :v, conceptEvalLike = :l, conceptEvalComment = :c',
...
const reportComplete = async () => {
  var message = '콘셉트 평가 영상 반응 정보를 업데이트 했습니다.';
