...
process.on('unhandledRejection', error => {
  const message = '[ERROR][그룹 배틀 평가] ' + error.message;
...
const crawling = async items => {
  console.info('그룹 배틀 직캠 반응 크롤링 시작!');
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.groupBattleDirectCamUrl || !item.groupBattleDirectCamUrl.length) {
      continue;
    }

    await page.goto(item.groupBattleDirectCamUrl,  {waitUntil: 'networkidle2'});
  ...
  console.info('그룹 배틀 직캠 반응 크롤링 완료!');
};
...
function store(item) {
  const params = {
    ...
    UpdateExpression: 'set groupBattleView = :gv, groupBattleLike = :gl, groupBattleComment = :gc',
...
const reportComplete = async () => {
  var message = '프로듀스48 그룹 배틀 직캠 영상 관련 정보를 업데이트 했습니다.';
